...
(define eval-Expression
	(lambda (Expression env) 
		...
		(PlusExpression (Token1 Token2 Expression1 Expression2 Token3)
			(if (equal? (eval-Expression Expression1 env) (eval-Expression Expression2 env))
                            ((if (equal? (eval-Expression Expression1 env) 'Int) 'Int (error "Type Error"))) (error "Type Error")))
                            
		(IfExpression (Token1 Token2 Expression1 Expression2 Expression3 Token3)
                         (if (equal? (eval-Expression Expression1 env) 'bool) (if (equal? (eval-Expression Expression1 env) (eval-Expression Expression2 env)) (eval-Expression Expression1 env) (error "Type Error")) (error "Type Error"))
		)
                (Identifier (Token) (if (null? (apply-env env Token)) (apply-env env Token) (unbox (apply-env env Token))))
                ;procedure
                ...
                (Assignment (Token1 Token2 Identifier Expression Token3)
                            'void)
		(else (error 'eval-Expression "Type Error")))))
(define run
(lambda ()
	(record-case root
